...
{:then status}
    {#if status === 'ACCEPTED'}
        <p>
            ...
            Please proceed with the rest of the transfer. When you click the "Next" button, the
            transfer will be submitted to the anchor server.
        </p>
    {:else}
        <p>
        ...
        <button
            ...
            on:click={getSep12Fields({authToken: $webAuthStore[homeDomain], homeDomain: homeDomain})}>Refresh status</button
        >
    {/if}
{/await}
